...
public final class Matrix {
	...
	public Matrix(GenerateMagicSquares magicSquares, int[] data) {
		magic_squares = magicSquares;
		this.data = data;
		this.equivalent_data = this.get_equivalence_class().get(0);
	}
	
	public Matrix(GenerateMagicSquares magicSquares, int[][] data_2d) {
		magic_squares = magicSquares;
	...
	public String toString() {
		...
		for (int i = 0; i < magic_squares.order; i++) {
			border += "--" + GenerateMagicSquares.str_repeat("-", max_term_size);
			border_between += "--" + GenerateMagicSquares.str_repeat("-", max_term_size);
		...
		for (int m = 0; m < magic_squares.order; m++) {
			...
			for (int n = 0; n < magic_squares.order; n++) {
				...
				result += " " + data[magic_squares.order*m+n] + GenerateMagicSquares.str_repeat(" ", padding_right)+ " |";
			}
			result += "\n";
	...
	public boolean equals(Object other) {
		return this.hashCode() == ((Matrix) other).hashCode() && 
			Arrays.equals(this.equivalent_data, ((Matrix) other).equivalent_data);
	}
